...
> 🟡 L2-理解 | 🎤🎤 中频面试
A company wants to migrate a large on-premises SQL Server application to AWS with minimal code changes. The application uses T-SQL stored procedures and triggers extensively. The company wants a managed database service.

Which solution should the solutions architect recommend?

- A. Migrate to Amazon RDS for SQL Server using DMS
- B. Refactor the application to use PostgreSQL on Aurora, rewriting all stored procedures
- C. Use Aurora PostgreSQL with Babelfish to run T-SQL with minimal code changes
- D. Run SQL Server on EC2 instances with Multi-AZ deployment

### Q4.24

> 🟡 L2-理解 | 🎤🎤 中频面试
A company's Redshift cluster experiences intermittent query queuing during monthly reporting when hundreds of analysts run queries simultaneously. The cluster size is otherwise adequate for normal operations.

Which feature should be enabled to handle the burst query load without resizing the cluster?

- A. Elastic Resize to temporarily add nodes during reporting periods
- B. Concurrency Scaling to add transient capacity for burst queries
- C. Classic Resize to change node types before each reporting period
- D. Redshift Spectrum to offload queries to S3
...
> **📊 Chapter 4 Summary**: 9 Knowledge + 12 Scenario + 5 Comparison = 26 questions
> 
