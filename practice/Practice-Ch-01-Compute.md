...
### Q1.19
A startup is deciding between compute options for a new microservice. The service receives unpredictable traffic — sometimes zero requests for hours, then a sudden burst of thousands of requests in seconds. Each request takes 2–3 seconds to process. The team has no dedicated operations staff.
...
### Q1.22
A company's workload consists of thousands of independent, short-lived tasks (2–5 seconds each) triggered by S3 object uploads. The volume of uploads varies from 10/minute overnight to 10,000/minute during peak hours. The processing logic is a simple Python script with no external dependencies.
...
- **B**: Reserved Instances require a 1–3 year commitment; unsuitable if the workload pattern could change.
...
**Why**: Attribute-based instance type selection lets you define requirements (e.g., "4–8 vCPUs, 16–32 GB memory") instead of specifying exact instance types. The ASG automatically selects the best matching type from all available families and generations, including newer instances as AWS releases them — no template updates needed.
...
**Why**: Lambda is ideal for variable, bursty traffic with zero-to-thousands swings. It scales instantly per-request, costs nothing when idle (no requests = $0), and requires no operations staff to manage. Each request takes 2–3 seconds — well within Lambda's capabilities.
...
**Why**: S3 event notifications can directly trigger Lambda on each object upload — no queue, no polling, no infrastructure. Lambda scales automatically per request, handles 2–5 second tasks perfectly, and costs nothing when uploads are at 10/minute overnight. This is the canonical "S3 → Lambda" event-driven pattern. Python support is native.
...
> **📊 Chapter 1 Summary**: 8 Knowledge + 10 Scenario + 4 Comparison = 22 questions
> 
