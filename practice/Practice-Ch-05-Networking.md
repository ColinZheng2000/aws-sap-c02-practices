...
**Why**: TGW is designed for this exact evolution — 3 VPCs today, thousands tomorrow. A hub-and-spoke model means adding a new VPC requires only one new attachment to the TGW (linear growth), vs VPC Peering which requires N×(N−1)/2 connections (quadratic growth). Choosing TGW from the start avoids a painful migration later.
...
> **📊 Chapter 5 Summary**: 10 Knowledge + 11 Scenario + 5 Comparison = 26 questions
> 
