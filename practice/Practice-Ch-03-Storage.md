...
- **0–30 days**: Frequent diagnostic access → keep in S3 Standard (not mentioned as a transition — it starts here).
- **30–120 days**: Occasional follow-up access (milliseconds retrieval acceptable) → Standard-IA.
- **120 days–1 year**: Accessed occasionally (minutes to hours retrieval acceptable) → Glacier Flexible Retrieval.
...
**Why**: DataSync is designed for online data transfer between on-premises and AWS (S3, EFS, FSx). With a 1 Gbps connection, transferring 80 TB would take approximately 8–9 days at full utilization (80 TB × 8 bits/byte ÷ 1 Gbps ≈ 178 hours), which fits within the 2-week window. DataSync supports incremental sync after the initial full transfer. Bandwidth throttling prevents saturation of the shared connection.
