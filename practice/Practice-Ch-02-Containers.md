...
tiers:
  ...
  scenario: 4
  comparison: 3
basedOn: "AWS-SAP-C02-Learning-Material.md 搂2"
services:
  - ECS
  - EKS
  - ECR
...
# Chapter 2 Practice: 馃摝 Containers

> **Instructions**: Complete Part A first. Do not scroll past the divider. Once finished, check Part B for answers.
> **Textbook**: `AWS-SAP-C02-Learning-Material.md` 鈥?Section 2 (ECS, EKS, ECR, Fargate) + Similar Service Comparison: Containers

---

# Part A 鈥?Questions

## 馃煝 Knowledge Check (3 questions)
...
- B. No EC2 instances to manage 鈥?serverless container deployment
...
## 馃煛 Scenario Analysis (4 questions)
...
- A. This architecture is invalid 鈥?Lambda cannot use container images
- B. This architecture is valid 鈥?Lambda supports container images up to 10 GB from ECR
...
- A. ECS 鈥?AWS-native, lower complexity, no Kubernetes knowledge needed
- B. EKS 鈥?more portable across clouds, standard Kubernetes API
- C. EKS on Fargate 鈥?serverless Kubernetes, best of both worlds
- D. Self-managed Kubernetes on EC2 鈥?maximum control
...
- A. Lambda 鈥?serverless, no infrastructure management, handles 30-minute tasks
- B. ECS on Fargate 鈥?Lambda has a 15-minute timeout; 30-minute tasks require ECS/Fargate
- C. Lambda with container images 鈥?container support removes the timeout limit
- D. ECS on EC2 鈥?Fargate also has a 15-minute timeout
...
### A2.1
**Correct: B** 鈥?No EC2 instances to manage 鈥?serverless container deployment.

**Why**: Fargate is the serverless compute engine for containers. You define the task (CPU, memory, container image) and Fargate runs it 鈥?no EC2 instances to provision, patch, or scale. This is the core value proposition: operational simplicity. You pay per task and per vCPU/GB used.
...
**馃摉 Textbook ref**: 搂2 鈥?ECS, "ECS + Fargate: No EC2 management needed"

---

### A2.2
**Correct: B** 鈥?Amazon ECR with Enhanced Scanning.
...
**馃摉 Textbook ref**: 搂2 鈥?ECR, "Image Scanning: Enhanced scanning with Amazon Inspector"

---

### A2.3
**Correct: B** 鈥?EKS on Fargate runs Kubernetes pods on serverless compute, with no node management.

**Why**: When you use EKS with Fargate, each Kubernetes pod runs on its own Fargate compute 鈥?there are no EC2 worker nodes to manage. You still use the standard EKS control plane (managed by AWS), but the data plane is fully serverless. You interact with your cluster using standard Kubernetes tools (kubectl, Helm).
...
**馃摉 Textbook ref**: 搂2 鈥?EKS, "EKS on Fargate: Serverless pods 鈥?no node management"
...
### A2.4
**Correct: B** 鈥?ECS on EC2.

**Why**: The security agent requires kernel module access and root on the host OS. Fargate is serverless 鈥?you have NO access to the underlying host. Only ECS on EC2 gives you control over the EC2 host operating system, allowing you to install kernel modules, security agents, and other host-level software while still using ECS for container orchestration.

**Why not the others**:
- **A**: Fargate provides no host access 鈥?you cannot install kernel modules.
...
**馃摉 Textbook ref**: 搂2 鈥?ECS + EC2, "You manage the cluster, more control"; 搂1 鈥?Similar Service Comparison, "OS/kernel control" row

---

### A2.5
**Correct: B** 鈥?ECS on Fargate with Service Auto Scaling.

**Why**: The scenarios fit Fargate: no server management, variable traffic, and task durations of 2鈥?0 minutes. Fargate with Service Auto Scaling handles the scaling automatically (scale out when traffic increases, scale in when it drops). Tasks can run for any duration on Fargate (no 15-minute limit like Lambda). This is simpler than ECS on EC2 (no EC2 management) and simpler than EKS (no Kubernetes complexity).

**Why not the others**:
- **A**: ECS on EC2 requires managing EC2 instances 鈥?"simplest possible configuration" points to Fargate.
- **C**: Lambda has a 15-minute max timeout 鈥?10-minute tasks are within limit, but Fargate is better for consistently long-running services (2-10 min each).
- **D**: EKS on Fargate introduces Kubernetes complexity that's unnecessary.

**馃摉 Textbook ref**: 搂1 鈥?Similar Service Comparison, "Lambda vs ECS vs Fargate vs EC2"

---

### A2.6
**Correct: B** 鈥?Amazon EKS.

**Why**: EKS is a managed Kubernetes control plane that is compatible with standard Kubernetes tooling 鈥?kubectl, Helm, existing manifests, and CI/CD pipelines that use kubectl commands work with EKS with minimal changes (primarily updating the kubeconfig to point to the EKS cluster endpoint). This is the primary reason organizations choose EKS over ECS: they already have Kubernetes investments.

**Why not the others**:
- **A**: ECS uses its own API and orchestration model 鈥?manifests and kubectl workflows don't transfer.
...
**馃摉 Textbook ref**: 搂2 鈥?EKS, "Azure Bridge: existing K8s investment"; Similar Service Comparison, "Existing investment" row

---

### A2.7
**Correct: B** 鈥?This architecture is valid 鈥?Lambda supports container images up to 10 GB from ECR.
...
- **C**: Lambda can use container images with or without VPC 鈥?VPC is not required.
- **D**: Lambda invocations are managed by Lambda, not ECS.

**馃摉 Textbook ref**: 搂1 鈥?Lambda, "Docker/Lambda: Container images up to 10 GB in ECR"
...
### A2.8
**Correct: A** 鈥?ECS 鈥?AWS-native, lower complexity, no Kubernetes knowledge needed.
...
- **C**: EKS on Fargate doesn't eliminate Kubernetes complexity 鈥?you still need to understand and manage Kubernetes constructs.
- **D**: Self-managed Kubernetes maximizes complexity 鈥?opposite of the team's goals.

**馃摉 Textbook ref**: 搂2 鈥?Similar Service Comparison, "ECS vs EKS"

---

### A2.9
**Correct: B** 鈥?ECS on Fargate 鈥?Lambda has a 15-minute timeout; 30-minute tasks require ECS/Fargate.

**Why**: This is a straightforward timeout comparison. Lambda max timeout = 15 minutes. Video processing = 30 minutes per task. Lambda is categorically not an option. ECS on Fargate has no task duration limit 鈥?tasks can run indefinitely. This is the correct choice for long-running workloads.

**Why not the others**:
- **A**: Lambda cannot run 30 minutes 鈥?this is factually wrong.
- **C**: Container image support does not change the 15-minute timeout limit.
- **D**: Fargate has no timeout limit for tasks 鈥?this statement is false.

**馃摉 Textbook ref**: 搂1 鈥?Similar Service Comparison, "Max runtime" row

---

### A2.10
**Correct: B** 鈥?Distribute ECS tasks across multiple AZs with distinctInstance placement and a multi-AZ ASG.
...
- **C**: Fargate also requires configuring multi-AZ (subnets in multiple AZs) 鈥?it's not automatically multi-AZ.
- **D**: Service Auto Scaling handles load-based scaling, not AZ failure resilience.

**馃摉 Textbook ref**: 搂2 鈥?ECS, "ECS + EC2: placement constraints/strategies"; 搂5 鈥?ALB

---

> **馃搳 Chapter 2 Summary**: 3 Knowledge + 4 Scenario + 3 Comparison = 10 questions
> 
